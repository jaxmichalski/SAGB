...
# x0,z0,vx0,vy0,vz0,w,theta
# outputs the x & z positions at 23.8' and 25'
def pitchProcess(start_state, target_ys):
	# converts the start state to metric
	start_state = metricConvert(start_state)
	y_start = 15.24 # starting y in meters
	w = start_state[5]
	theta = start_state[6]

	# extracts the elements of the start state needed for coordinates
	start_coords = np.array([start_state[0],y_start] + start_state[1:5])

	# setting dt
	dt = 0.5 / 1000

	# sort the target_ys array
	target_ys = sorted(target_ys, reverse=True)
	results = []
	# fencepost with the first target_ys
	results.append(iterativeIntegrate(start_coords, w, theta, dt, target_ys[0]))
	# compute the rest of the target_ys
	for y in target_ys[1:]:
		results.append(iterativeIntegrate(results[-1], w, theta, dt, y))
...
def fileProcess(filename):
	...
	# x0, z0, vx0, vy0, vz0, spin_dir, spin_rate
	q_indices = [45,47,48,49,50,61,62]
...
def main():
	data = fileProcess("Pitch_Tables/pitch_table.16.csv")
	...
	for d in data:
		...
		if counter % 100 == 0:
			print counter
		new_result = d[:3]
		new_result += pitchProcess(d[3:], [7.62, 7.215])
		results.append(new_result)

	outfile = open('pitch_table_16_processed.txt', 'w')
	for r in results:
